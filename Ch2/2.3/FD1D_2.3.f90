!========================================================
!   Purpose: 1D FDTD simulation of a frequency dependent
!            material
!
!   Methods: FDTD + Fourier Transform
!       
...
PROGRAM main
    ...
    REAL(8) :: ga(Ncell), gb(Ncell), gc(Ncell)
    REAL(8) :: ix(Ncell), sx(Ncell)
    ...
    freq = (/ 50.0E6, 200.0E6, 500.0E6, 0.0, 0.0 /)
    arg = 2 * pi * freq * dt
    ...
    dielectric: do k = Kstart, Ncell
        ga(k) = 1.0 / (eps + sigma * dt / epsz + chi1 * dt &
                       / tau)
        gb(k) = sigma * dt / epsz
        gc(k) = chi1 * dt / tau
    end do dielectric
    ...
    open(2, file='fourier/1000step/amp0', status='unknown')
    do k = 1, Ncell
        write(2, *) ampn(1, k)
    end do
    open(3, file='fourier/1000step/amp1', status='unknown')
    do k = 1, Ncell
        write(3, *) ampn(2, k)
    end do
    open(4, file='fourier/1000step/amp2', status='unknown')
    do k = 1, Ncell
        write(4, *) ampn(3, k)
    end do
END PROGRAM main
